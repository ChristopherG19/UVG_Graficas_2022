...
class Render(object):
    ...
    def glVertex(self, x, y, colorp=None):
        ...
        if (xComp or yComp):
            ...
            x2 = (x + 1) * trunc(self.ViewPW/2) + self.InX
            y2 = (y + 1) *trunc(self.ViewPH/2) + self.InY

            self.glpoint(trunc(x2), trunc(y2), colorp or self.VP_Color)
    ...
    def glLine(self, x0, y0, x1, y1, colorp=None):
        ...
        if (xComp or yComp):
            ...
            x0 = (x0 + 1) * trunc(self.ViewPW/2) + self.InX
            y0 = (y0 + 1) *trunc(self.ViewPH/2) + self.InY
            x1 = (x1 + 1) * trunc(self.ViewPW/2) + self.InX
            y1 = (y1 + 1) *trunc(self.ViewPH/2) + self.InY
        ...
        for x in range (trunc(x0), trunc(x1)+1):
            
            if limEm:
                self.glpoint(trunc(y),trunc(x), colorp)
            else: 
                self.glpoint(trunc(x),trunc(y), colorp) 
    ...
    def line(self, x0, y0, x1, y1, colorp=None):
